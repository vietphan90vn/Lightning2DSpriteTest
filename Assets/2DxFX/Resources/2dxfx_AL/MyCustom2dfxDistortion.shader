...
Shader "2DxFX/Custom/MyCustom2dfxDistortion"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata_t IN)
            {
                ...
                OUT.color = IN.color;
				OUT.refl = ComputeScreenPos (OUT.vertex);
                return OUT;
            }
            ...
            float4 frag(v2f IN): COLOR
            {
				...
				float4 p1 = IN.refl;
                p1.x = p1.x + sin(p1.y * _OffsetX + _WaveTimeX) * _DistanceX;
                p1.y = p1.y + cos(p1.x * _OffsetY + _WaveTimeY) * _DistanceY;

				float4 reflColor = tex2Dproj(_ReflectionTex, UNITY_PROJ_COORD(p1));
                float4 mainColor = tex2D(_MainTex, p) * IN.color;
				mainColor = mainColor * reflColor;
                mainColor.a -= _Alpha;
                return mainColor;
            }
            ENDCG
